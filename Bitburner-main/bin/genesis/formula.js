...
export async function main(ns) {
  ...
  function info() {
    // log basic info
    ns.print(
      `Home ram \t ${ns.formatRam(ns.getServerMaxRam("home"))} / ${ns.formatRam(
        getHomeRam()
      )}`
    );
    ns.print(
      `Netw ram \t ${ns.formatRam(NmapTotalRam(ns))} / ${ns.formatRam(
        NmapFreeRam(ns)
      )}`
    );
  ...
  function calculateGrowThreads(server, cores) {
    ...
    return Math.ceil(
      ns.growthAnalyze(server, Math.ceil(100 - mulitplier)),
      cores
    );
  }
  ...
  function createTargetList() {
    ...
    TARGET_LIST.sort(function (a, b) {
      return a.threadsHome - b.threadsHome;
    });
  ...
  function installScripts() {
    if (TARGET_LIST.length === 0) {
      TARGET_LIST.push({
        ...
        ramHome:
          ns.getScriptRam(SCRIPT.grow) * calculateGrowThreads("n00dles", 1),
        ramNetw:
          ns.getScriptRam(SCRIPT.grow) * calculateGrowThreads("n00dles", 1),
      });
    }
