...
import { scriptPath, scriptStart, colorPrint } from "./settings";
...
export async function main(ns) {
  ...
  let CHANCE_UPPERBAND = HACK_CHANCE,
    CHANCE_LOWERBAND = 0.79;
  ...
  function weakCondition(server) {
    return (
      ...
      ns.getServerMinSecurityLevel(server) + SECURITY_PATCH
    );
  }

  function calculateWeakThreads(server) {
    // caculates number of threads for weak
    let serverSecutityDiff = Math.ceil(
  ...
  function calculateHackThreads(server) {
    ...
    return Math.ceil(
      ns.hackAnalyzeThreads(server, ns.getServerMaxMoney(server) * HACK_PROCENT)
    );
  }
      ...
      if (ns.hackAnalyzeChance(server) >= CHANCE_UPPERBAND) {
        if (weakCondition(server)) {
          list.push({
            hostname: server,
            action: "weak",
            threads: calculateWeakThreads(server),
          });
        } else if (growCondition(server)) {
          list.push({
            hostname: server,
            action: "grow",
            threads: calculateGrowThreads(server),
          });
        } else {
          list.push({
            hostname: server,
            action: "hack",
            threads: calculateHackThreads(server),
          });
        }
      }
    }
      ...
      return [
        {
          hostname: "n00dles",
          action: "grow",
          threads: calculateGrowThreads("n00dles"),
        },
      ];
    } else {
  ...
  function preWeak() {
    ...
    if (!ns.scriptRunning(SCRIPT.weak, "home") && CHANCE_LOWERBAND > -1) {
      ...
      for (let target of NmapMoneyServers(ns)) {
        if (
          ns.hackAnalyzeChance(target) <= CHANCE_UPPERBAND &&
    ...
    if (NmapFreeRam(ns) === NmapTotalRam(ns)) {
      ...
      for (let target of targets) {
        if (target.action === "weak") {
          if (
            !distributeAcrossNetwork(
              SCRIPT.weak,
              target.threads,
              target.hostname
            )
          ) {
            ns.print("W... - " + target.hostname);
            break;
          } else {
            ns.print("WEAK - " + target.hostname);
          }
          ...
          if (
            ...
            ns.print("G... - " + target.hostname);
            break;
          } else {
            ns.print("GROW - " + target.hostname);
          }
          ...
          if (
            ...
            colorPrint(ns, "white", "H... - " + target.hostname);
            break;
          } else {
            colorPrint(ns, "white", "HACK - " + target.hostname);
          }
        }
      }
    }
