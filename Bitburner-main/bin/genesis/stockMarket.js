...
export async function main(ns) {
  ...
  function displayLog() {
    ...
    PORTFOLIO.forEach((stock) => {
      ...
      let sharesProcent =
        ns.stock.getPosition(ticker)[0] / ns.stock.getMaxShares(ticker);
      let profit = Math.round(
        ns.stock.getSaleGain(ticker, ns.stock.getPosition(ticker)[0], "Long") -
          ns.stock.getPosition(ticker)[1] * ns.stock.getPosition(ticker)[0]
      );
      let log =
        ticker +
        "\t" +
        forcast +
        "\t" +
        ns.formatPercent(volatility, 2) +
        "\t" +
        ns.formatPercent(sharesProcent, 0) +
        "\t" +
  ...
  function displayStatus() {
    ...
    PORTFOLIO.forEach((stock) => {
      let sharesOwnedProcent = Math.round(
        (ns.stock.getPosition(stock.sym)[0] /
          ns.stock.getMaxShares(stock.sym)) *
          100
      );
      let profit = Math.round(
        ns.stock.getSaleGain(
          ...
          ns.stock.getPosition(stock.sym)[1] *
            ns.stock.getPosition(stock.sym)[0]
      );
      ...
      ns.print(
        "volati\t" +
          (ns.stock.getVolatility(stock.sym) * 100).toPrecision(3) +
          "%"
      );
      ns.print("shares \t" + sharesOwnedProcent + "%");
      profit >= 0
        ? ns.print("profit\t" + ns.formatNumber(profit))
        : colorPrint(ns, "red", "losses\t" + ns.formatNumber(profit));
    });
  }
  ...
  function buyShares() {
    ...
    for (let stock of PORTFOLIO) {
      let availableShares =
        ns.stock.getMaxShares(stock.sym) - ns.stock.getPosition(stock.sym)[0];
      ...
      if (availableShares > 0 && availableMoney > BALANCE_TRIGGER_THRESHOLD) {
        if (spendableMoney > priceMaxShares) {
          ...
          // buy parts
          let partShares = Math.floor(
            spendableMoney / ns.stock.getPrice(stock.sym)
          );
