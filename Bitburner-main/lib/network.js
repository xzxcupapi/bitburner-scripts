import { scriptPath } from "./settings";
...
export function NmapRamServers(ns) {
  ...
  for (let server of servers) {
    if (ns.getServerMaxRam(server) > 0 && !excludedServers.includes(server)) {
      list.push(server);
    }
  }
  return list;
...
export function copyHackScripts(ns, server) {
  ...
  if (
    !ns.fileExists(path.grow, server) ||
...
export async function installBackdoor(ns, server) {
  if (
    ...
    getServerPath(ns, server).forEach((node) => {
