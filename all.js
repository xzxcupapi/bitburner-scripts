...
export async function main(ns) {
  ...
  for (let server of allServers) {
    ...
    // Hanya menambahkan server yang tidak menjalankan stolen.js dan tidak memiliki akses root
  ...
  // Mengurutkan server berdasarkan level hacking yang diperlukan dan akses root
  serverInfo.sort((a, b) => {
    // Pertama, urutkan berdasarkan akses root
    if (a.hasRootAccess === b.hasRootAccess) {
      // Jika akses root sama, urutkan berdasarkan level hacking yang diperlukan
      return a.requiredHackingLevel - b.requiredHackingLevel;
    }
    // Jika satu memiliki akses root dan yang lainnya tidak
    return a.hasRootAccess ? -1 : 1; // Urutkan yang memiliki akses root terlebih dahulu
  });
  ...
  for (let info of serverInfo) {
    ...
    result += `  RAM yang tersedia: ${info.ramAvailable} GB\n`; // Menampilkan RAM
  ...
  // Menyimpan hasil ke file All.txt
  await ns.write("All.txt", result, "w"); // Menyimpan hasil ke file

  ns.tprint("Informasi server telah disimpan di All.txt");
}
