...
export async function main(ns) {
  const logsToDisable = ["sleep", "getServerMoneyAvailable"];
  ...
  if (!is4SigmaAvailable) {
    ns.tprint(
      "ERROR Purchase the 4Sigma API access, otherwise this script is not doing anything"
    );
    ns.exit();
  }
...
async function trader(ns) {
  ...
  const debugPrint = (sym, isLong) => {
    ...
    if (isLong != undefined) {
      symStr = (isLong ? "📈" : "📉") + sym;
      endStr = ` 💲${ns.nFormat(investment, "000.00a")} 💲${ns.nFormat(
        income,
        "000.00a"
      )} ${income > 0 ? "👍" : "👎"}`;
    }
    ns.print(
      `${symStr}\t${stock.getForecast(sym).toFixed(2)} - ${(
        stock.getVolatility(sym) * 100
      ).toFixed(2)} ${ns.nFormat(getSymbolPoint(stock, sym), "00.00")}` + endStr
    );
  };
  ...
  for (let sym of symbols) {
    ...
    const amountToBuy =
      stock.getMaxShares(sym) * MAX_STOCK_OWNED_PERCENT - shares - sharesShort;
    if (isLong) {
      const amountToAfford = Math.min(
        amountToBuy,
        Math.floor(money / stock.getAskPrice(sym))
      );
      if (amountToAfford > 0) {
      ...
      const amountToAfford = Math.min(
        amountToBuy,
        Math.floor(money / stock.getBidPrice(sym))
      );
      if (amountToAfford > 0) {
...
function getOwnedSymbols(stock) {
  ...
  return symbols.sort(
    (a, b) => getPossibleIncome(stock, a.sym) - getPossibleIncome(stock, b.sym)
  );
}
...
function sortAndFilterSymbols(stock) {
  const filteredSymbols = stock
    ...
    .filter((sym) => {
      ...
      const [shares, avgPx, sharesShort, avgPxShort] = stock.getPosition(sym);
      return (
        stock.getMaxShares(sym) * MAX_STOCK_OWNED_PERCENT >
        Math.max(shares, sharesShort)
      );
    });
  return filteredSymbols.sort(
    (a, b) => getSymbolPoint(stock, b) - getSymbolPoint(stock, a)
  );
}
...
const dateSuffix = (ns) =>
  `${ns.nFormat(new Date().getMonth(), "00")}-${ns.nFormat(
    new Date().getDate(),
    "00"
  )}`;
/** @param {NS} ns */
function readLogs(ns) {
  ...
  if (ns.fileExists(incomeFile))
    earnedMoney = JSON.parse(ns.read(incomeFile)).income;
}
...
async function logBuySell(ns, symObj, isBuy) {
  ...
  const date = `${ns.nFormat(dateObj.getHours(), "00")}:${ns.nFormat(
    dateObj.getMinutes(),
    "00"
  )}`;
...
function printLogs(ns) {
  ...
  for (let log of logs) {
    ...
    const amount =
      short > 0
        ? `SHORT ${ns.nFormat(short, "000.0a")}`
        : `LONG  ${ns.nFormat(long, "000.0a")}`;
