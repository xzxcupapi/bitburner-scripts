...
const infiltrationGames = [
  ...
  },
  {
    ...
    play: function (screen) {
      ...
      const lastArrowIndex = Math.max(
        ...["↑", "↓", "←", "→"].map((arrow) => arrowsText.lastIndexOf(arrow))
      );
  ...
  },
  {
    ...
    init: function (screen) {
      console.log("Slash game initialized");
      state.game.data = "wait";
      state.game.guardingTimeStart = new Date().getTime();
      state.game.predictedPreparationTime = Math.random() * 3250 + 1500; // Approximate preparation time
    },
    play: function (screen) {
      if (state.game.data === "done") return; // Exit if game is already done

      const currentTime = new Date().getTime();
      const elapsedTime = currentTime - state.game.guardingTimeStart;

      // Fetch the current game phase from the screen
      ...
      if (
        ...
        (-1 !== data.indexOf("Preparing?") || -1 !== data.indexOf("ATTACKING!"))
      ) {
        console.log("Text detection trigger activated");
